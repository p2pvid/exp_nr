...
function tuplesToBuffer(tuples) {
    return fromBuffer(buffer_1.Buffer.concat(tuples.map((tup) => {
        const proto = protoFromTuple(tup);
        let buf = buffer_1.Buffer.from(varint_1.default.encode(proto.code));
        if (tup.length > 1) {
            buf = buffer_1.Buffer.concat([buf, tup[1]]); // add address buffer
        }
        return buf;
    })));
}
...
function fromBuffer(buf) {
    ...
    if (err)
        throw err;
    return buffer_1.Buffer.from(buf); // copy
}
